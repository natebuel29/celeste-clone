...
LRESULT CALLBACK windows_window_callback(HWND window, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_MBUTTONUP:
    {
        ...
        int mouseCode =
            (msg == WM_LBUTTONDOWN || msg == WM_LBUTTONUP) ? VK_LBUTTON : (msg == WM_MBUTTONDOWN || msg == WM_MBUTTONUP) ? VK_MBUTTON
                                                                                                                         : VK_RBUTTON;
...
bool platform_create_window(int width, int height, char *title)
{
    ...
    // Fake Window Initializing OpenGl
    {
        ...
        if (window == NULL)
        {
            NB_ASSERT(false, "Failed to create Windows window");
            return false;
        }
        ...
        if (!fakeDC)
        {
            NB_ASSERT(false, "Failed to get HDC");
            return false;
        }
        ...
        if (!pixelFormat)
        {
            NB_ASSERT(false, "Failed to choose pixel format");
            return false;
        }

        if (!SetPixelFormat(fakeDC, pixelFormat, &pfd))
        {
            NB_ASSERT(false, "Failed to set pixel format");
            return false;
        }
        ...
        if (!fakeRC)
        {
            NB_ASSERT(false, "Failed to create Render context");
            return false;
        }

        if (!wglMakeCurrent(fakeDC, fakeRC))
        {
            NB_ASSERT(false, "Failed to make current");
            return false;
        }
    ...
    // Actual OpenGL initialization
    {
        ...
        if (!wglChoosePixelFormatARB(dc, pixelAttribs,
                                     ...
                                     &numPixelFormats))
        {
            NB_ASSERT(0, "Failed to wglChoosePixelFormatARB");
            return false;
        }
        ...
        {
            NB_ASSERT(0, "Failed to SetPixelFormat");
            return true;
        }
        ...
        if (!rc)
        {
            NB_ASSERT(0, "Failed to crate Render Context for OpenGL");
            return false;
        }

        if (!wglMakeCurrent(dc, rc))
        {
            NB_ASSERT(0, "Faield to wglMakeCurrent");
            return false;
        }
    }
...
void *platform_load_gl_function(char *funName)
{
    ...
    if (!proc)
    {
        ...
        if (!proc)
        {
            NB_ASSERT(false, "Failed to load gl function %s", "glCreateProgram");
            return nullptr;
        }
    }
...
void *platform_load_dynamic_library(char *dll)
{
    ...
    NB_ASSERT(result, "failed to load dll: %s", dll);
    return result;
}
...
void *platform_load_dynamic_function(void *dll, char *funName)
{
    ...
    NB_ASSERT(proc, "Failed to load function: %s from DLL", funName);
...
bool platform_free_dynamic_library(void *dll)
{
    ...
    NB_ASSERT(freeResult, "Failed to free library");
...
void platform_update_audio(float dt)
{
    for (int soundIdx = 0; soundIdx < soundState->playingSounds.count; soundIdx++)
    {
        ...
        NB_ASSERT(sound.size > 0, "Sound has no Samples Size: %d", sound.size);
        NB_ASSERT(sound.data, "Sound has no Data!");
