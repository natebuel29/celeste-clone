...
#define NB_TRACE(msg, ...) _log("TRACE: ", msg, TEXT_COLOR_GREEN, ##__VA_ARGS__);
#define NB_WARN(msg, ...) _log("WARN: ", msg, TEXT_COLOR_YELLOW, ##__VA_ARGS__);
#define NB_ERROR(msg, ...) _log("ERROR: ", msg, TEXT_COLOR_RED, ##__VA_ARGS__);
#define NB_FATAL(msg, ...) _log("TRACE: ", msg, TEXT_COLOR_BRIGHT_RED, ##__VA_ARGS__);
...
struct Array
{
    ...
    T &operator[](int idx)
    {
        NB_ASSERT(idx >= 0, "idx negative!");
        NB_ASSERT(idx < count, "Idx out of bounds!");
        return elements[idx];
    }

    int add(T element)
    {
        NB_ASSERT(count < maxElements, "Array Full!");
...
BumpAllocator make_bump_allocator(size_t size)
{
    ...
    {
        NB_ASSERT(false, "Failed to allocate memory");
    }
...
char *bump_alloc(BumpAllocator *bumpAllocator, size_t size)
{
    ...
    {
        NB_ASSERT(false, "BumpAllocator is full");
    }
...
{
    NB_ASSERT(filePath, "No filePath supplied!");
...
long get_file_size(char *filePath)
{
    ...
    if (!file)
    {
        NB_ERROR("Failed opening File: %s", filePath);
        return 0;
    }
...
{
    NB_ASSERT(filePath, "No filePath supplied!");
    NB_ASSERT(fileSize, "No fileSize supplied!");
    NB_ASSERT(buffer, "No buffer supplied!");
...
bool copy_file(char *fileName, char *outputName, char *buffer)
{
    ...
    if (!outputFile)
    {
        NB_ERROR("Failed opening File: %s", outputName);
        return false;
    }
