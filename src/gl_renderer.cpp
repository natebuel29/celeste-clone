...
GLuint gl_create_shader(int shaderType, char *shaderPath, BumpAllocator *transientStorage)
{
    ...
    if (!shaderHeader)
    {
        NB_ASSERT(false, "Failed to load shader_header.h");
        return 0;
    }
    if (!shaderSource)
    {
        NB_ASSERT(false, "Failed to load shader: %s", shaderPath);
        return 0;
    }
    ...
    // Test if Shader compiled successfully
    {
        ...
        if (!success)
        {
            ...
            NB_ASSERT(false, "Failed to compile %s Shader, Error: %s", shaderPath, shaderLog);
            return 0;
        }
    }
...
bool gl_init(BumpAllocator *transientStorage)
{
    ...
    {
        NB_ASSERT(false, "Failed to create Shaders")
        return false;
    }
            ...
            timestampFrag > glContext.shaderTimestamp)
        {
            ...
            // Validate if program works
            {
                ...
                if (!programSuccess)
                {
                    ...
                    NB_ASSERT(0, "Failed to link program: %s", programInfoLog);
                    return;
                }
            }
